...
class Map extends React.Component {
    render() {
        ...
        function findCityCenter(cityCoords, index) {
            let length = 0;
            return cityCoords.map(coords => {
                length += coords.length;
                return coords.map(coord => coord[index]).reduce(reducer);
            }).reduce(reducer) / length;
        }
        const getCityCenter = (cityCoords) => [findCityCenter(cityCoords, 1), findCityCenter(cityCoords, 0)]; // координаты поменяны местами для встраивания в карту
